...
{
public:
  FormatConverter() {vision_detections = message_info::msg::VisionDetections();}

  void append_frame(const SSL_DetectionFrame & packet_detection)
  {
    auto detection_frame = message_info::msg::DetectionFrame();
  ...
  message_info::msg::VisionDetections get_frame() {return this->vision_detections;}

private:
  message_info::msg::DetectionBall convert_to_ball_topic(const SSL_DetectionBall & raw_ball)
  {
    auto detection_ball = message_info::msg::DetectionBall();

    detection_ball.pose.x = raw_ball.x() * TO_METER;
    detection_ball.pose.y = raw_ball.y() * TO_METER;
    //RCLCPP_INFO(this->get_logger(),"%f",detection_ball.pose.x);
    //std::cout<<detection_ball.pose.x;
    return detection_ball;
  }

  message_info::msg::DetectionRobot convert_to_robot_topic(const SSL_DetectionRobot & raw_robot)
  {
    auto detection_robot = message_info::msg::DetectionRobot();
  ...
  message_info::msg::VisionDetections vision_detections;
};
...
public:
  ...
  : Node("Masuo_receiver"), converter()
  {
    ...
    pub_geometry =
      this->create_publisher<message_info::msg::VisionGeometry>("vision_geometry", 1);
    pub_detection =
      this->create_publisher<message_info::msg::VisionDetections>("vision_detections", 1);

    this->declare_parameter("vision_addr", "224.5.23.2");
    this->declare_parameter("vision_port", 10020);
    ...
    receiver_info = std::unique_ptr<MulticastReceiver>(new MulticastReceiver(host, port));
  }
...
private:
  void timer_callback()
  {
    ...
    while (receiver_info->available()) {
      //RCLCPP_INFO(this->get_logger(),"%s","Hello");
      std::vector<char> buf(2048);
      const size_t size = receiver_info->receive(buf);
  ...
  {
    auto geometry = message_info::msg::VisionGeometry();
    ...
    for (auto line = lines.begin(); line != lines.end(); line++) {
      auto line_segment = message_info::msg::FieldLineSegment();
    ...
    for (auto arc = arcs.begin(); arc != arcs.end(); arc++) {
      auto circular_arc = message_info::msg::FieldCircularArc();
  ...
  rclcpp::Publisher<message_info::msg::VisionGeometry>::SharedPtr pub_geometry;
  rclcpp::Publisher<message_info::msg::VisionDetections>::SharedPtr pub_detection;
  rclcpp::TimerBase::SharedPtr timer_;

  std::unique_ptr<MulticastReceiver> receiver_info;
