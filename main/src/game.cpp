...
void Game::test(){
    ...
    message_info::msg::Role role;

    Role::decision(ball,frame,goal,role);

    send.commands.push_back(Attacker::main(ball,frame.blue_robots[role.attacker],goal)); //アタッカーメインプログラム
    send.commands.push_back(Offense::main(ball,frame.blue_robots[role.offense[0]],frame.blue_robots[role.attacker] ,goal)); 
    send.commands.push_back(Goalie::main(ball,frame.blue_robots[role.goalie],goal));
    send.commands.push_back(Defense::main(ball,frame.blue_robots[role.defense[0]],goal));
